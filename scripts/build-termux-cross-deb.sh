...
cp -r src/r2ofrak "$STAGE$PREFIX/lib/python3.12/site-packages/"

# ── 3. Create entry point scripts ────────────────────────────────
PY_SITE="$PREFIX/lib/python3.12/site-packages"

cat > "$STAGE$PREFIX/bin/r2ofrak" << WRAPPER
...
main()
WRAPPER
chmod 755 "$STAGE$PREFIX/bin/r2ofrak"

cat > "$STAGE$PREFIX/bin/r2ofrak-tui" << WRAPPER
#!/data/data/com.termux/files/usr/bin/env python3
import sys, os
sys.path.insert(0, '${PY_SITE}')
...
main()
WRAPPER
chmod 755 "$STAGE$PREFIX/bin/r2ofrak-tui"
...
echo "[+] R2OFRAK installed! Run: r2ofrak-tui"
POSTINST
